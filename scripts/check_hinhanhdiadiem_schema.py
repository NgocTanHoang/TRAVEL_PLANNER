#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Script kiểm tra cấu trúc bảng HINHANHDIADIEM"""
import sqlite3
...
cursor.execute('PRAGMA table_info(HINHANHDIADIEM)')
columns = cursor.fetchall()

print('='*100)
print('CÁC TRƯỜNG CỦA BẢNG HINHANHDIADIEM (theo thứ tự trong database)')
print('='*100)
...
cursor.execute("PRAGMA foreign_key_list(HINHANHDIADIEM)")
fks = cursor.fetchall()
if fks:
    print('\nForeign Keys:')
    for fk in fks:
...
cursor.execute("SELECT name, sql FROM sqlite_master WHERE type='index' AND tbl_name='HINHANHDIADIEM'")

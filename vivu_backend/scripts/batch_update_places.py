"""
Batch update script for places from row 58 onwards
- Geocode addresses to get lat/lon
- Fix incorrect loaiDiaDiem
- Add missing information
- Update amenities and descriptions
"""
import os
import sys
import django
...
# Database path
db_path = 'vivudb.sqlite3'
if not os.path.exists(db_path):
    db_path = os.path.join('..', 'vivudb.sqlite3')
...
def geocode_address(address: str, province_name: str = "") -> Optional[Dict[str, float]]:
    ...
    if province_name:
        coords = geocode_with_osm(address)
        if coords:
            return coords
...
def fix_loai_dia_diem(ten_dia_diem: str, loai_hien_tai: str, dia_chi: str) -> str:
    ...
    khach_san_keywords = ['khách sạn', 'hotel', 'resort', 'condotel', 'homestay', 'nhà nghỉ']
    nha_hang_keywords = ['nhà hàng', 'restaurant', 'quán ăn', 'café', 'cafe', 'bar', 'pub']
    giai_tri_keywords = ['vui chơi', 'giải trí', 'công viên', 'park', 'khu vui chơi', 'amusement']
    mua_sam_keywords = ['chợ', 'market', 'trung tâm thương mại', 'mall', 'siêu thị', 'supermarket']
    dia_danh_keywords = ['chùa', 'đền', 'lăng', 'bảo tàng', 'museum', 'temple', 'pagoda', 'di tích']
    ...
    return loai_hien_tai  # Keep current if no fix needed
...
def generate_amenities(loai_dia_diem: str, ten_dia_diem: str) -> str:
    ...
    if loai_dia_diem == 'khach_san':
        ...
        if 'resort' in ten_lower or 'resort' in ten_lower:
...
def improve_description(ten_dia_diem: str, loai_dia_diem: str, dia_chi: str, mo_ta_hien_tai: str) -> str:
    ...
    if mo_ta_hien_tai and len(mo_ta_hien_tai.strip()) > 50:
        return mo_ta_hien_tai  # Keep existing if it's substantial
...
def update_place(cursor: sqlite3.Cursor, rowid: int, row_data: Tuple) -> bool:
    ...
    try:
        ...
        province_name = province_result[0] if province_result else ""
        
        # 1. Fix coordinates if missing or zero
        ...
        if fixed_loai != loai_dia_diem:
            ...
            needs_update = True
            loai_dia_diem = fixed_loai  # Use fixed type for subsequent logic
    ...
    try:
        ...
        while True:
            ...
            for row in batch:
                ...
                if update_place(cursor, rowid, row_data):
                    updated += 1
                
                # Progress update
                if processed % 100 == 0:
                    print(f"Processed: {processed}/{total_count} | Updated: {updated} | "
                          f"Coords: {stats['updated_coords']} | Type: {stats['updated_type']} | "
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Batch update places from row 58 onwards')
    ...
    parser.add_argument('--test', action='store_true', help='Test mode: process only 10 places')
    
    args = parser.parse_args()
    
    if args.test:
        print("TEST MODE: Processing only 10 places")
        batch_update_places(start_rowid=args.start, limit=10)
    else:

...
class LangSmithConfig:
    ...
    def _setup_environment(self):
        ...
        # Project name - chỉ set nếu có trong env, nếu không thì để LangSmith tự động tạo
        project = os.getenv('LANGCHAIN_PROJECT', '')
        if project:
            os.environ['LANGCHAIN_PROJECT'] = project
        # Nếu không có project trong env, không set -> LangSmith sẽ dùng default
    ...
    def project_name(self) -> Optional[str]:
        """Get LangSmith project name. Returns None if not set to let LangSmith auto-create."""
        project = os.getenv('LANGCHAIN_PROJECT', '')
        if project:
            return project
        # Trả về None để LangSmith tự động tạo project
        return None
    ...
    def get_runnable_config(self, tags: Optional[list] = None, metadata: Optional[dict] = None):
        ...
        config_tags = ['travel-planner']
        # Chỉ thêm project name vào tags nếu có
        if self.project_name:
            config_tags.append(self.project_name)
        if tags:
        ...
        # Chỉ thêm project vào metadata nếu có
        if self.project_name:
            config_metadata['project'] = self.project_name
        if metadata:
